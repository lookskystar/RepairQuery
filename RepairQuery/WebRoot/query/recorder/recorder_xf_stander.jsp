...
<script type="text/javascript">
	...
	$(function(){
         ...
         <c:forEach items="${units}" var="unit" varStatus="index">
         	d.add(${index.index+1},0,'${unit.firstunitname }','<%=basePath%>recorderAction!queryRecorder.action?jcStype=${jcStype }&jcNum=${jcNum }&xcxc=${xcxc }&fristUnit=${unit.firstunitid }&rjhmId=${datePlan.rjhmId }&jwdCode=${jwdCode}');
 		 </c:forEach>
 		d.add('t_0',-1,'检修班组');//班组树形
 		<c:forEach items="${bzs}" var="bz" varStatus="index">
         	d.add('t_${index.index+1}','t_0','${bz.proteamname }','<%=basePath%>recorderAction!queryInfoByBz.action?jcStype=${jcStype }&jcNum=${jcNum }&xcxc=${xcxc }&teamId=${bz.proteamid }&rjhmId=${datePlan.rjhmId }&jwdCode=${jwdCode}','${bz.proteamname }','_blank');
 		</c:forEach>
 ...
 <tr>
	...
	<table width="864" border="0" align="center" cellspacing="0" vspace="0" id="datatabel">
		<tr style="line-height:40px;height: 40px;background-color: lightblue;font-weight: bolder;">
			...
			<td class="tbCELL1" align="center" style="white-space:nowrap" width="8%">大部件</td>
			<td class="tbCELL1" align="center" style="white-space:nowrap; min-width" width="12%">检修部件</td>
			<!-- <td class="tbCELL1" align="center">项目编号</td> -->
			<td class="tbCELL1" align="center" style="white-space:nowrap" width="20%">检修项目</td>
			<td class="tbCELL1" align="center" style="white-space:nowrap" width="20%">部位</td>
		...
		<c:forEach items="${itemRelation}" var="item">
		<tr>
			<td class="tbCELL1" align="center" name="firstname">${item.firstUnitName }&nbsp;</td>
			<td class="tbCELL1" align="center" name="secondname">${item.fixItem}&nbsp;</td>
			<td class="tbCELL1" align="center" name="fixitemname">${item.fixContent}&nbsp;</td>
			<td class="tbCELL1" align="center">${item.position }&nbsp;</td>
			<td class="tbCELL1" align="center">${item.fixSituation }${item.unit }&nbsp;</td>
		    <td class="tbCELL1" align="center">${item.fixEmp}</td>
            <td class="tbCELL1" align="center">${item.lead }</td>
            <td class="tbCELL1" align="center">${item.qi }</td>
            <td class="tbCELL1" align="center">${item.tech }</td>
            <td class="tbCELL1" align="center">${item.commitLead }</td>
            <td class="tbCELL1" align="center">${item.accepter }</td>
		</tr>
		</c:forEach>
	</table>
  </td>
