...
<ul id="nav">
    ...
    <li><a href="<%=basePath%>recorderAction!findJCPJ.action?rjhmId=${datePlan.rjhmId}&jwdCode=${jwdCode}" style="background-color:#fff;color:blue;">◇配件记录</a></li>
 		...
 		<c:forEach var="map" items="${map}">
 		  ...
 		  <c:if test="${!empty map.value}">
 		    <c:forEach var="pj" items="${map.value}" varStatus="s">
 		      ...
 		      <td class="tbCELL1" align="center" style="white-space:nowrap" width="20%"><a href="<%=basePath %>recorderAction!zxFittingDetailViewNew.action?rjhmId=${datePlan.rjhmId }&staticId=${pj.pjsid}&jwdCode=${jwdCode}" target="_blank">${pj.pjName}</a></td>
 		      ...
 		      <td class="tbCELL1" align="center" style="word-break:normal;word-wrap: break-word;" width="50%">
 		        <c:if test="${!empty pj.dynamicInfos}">
 		          <c:forEach var="dy" items="${pj.dynamicInfos}">
 		            <a href="recorderAction!findPjRecordByPjNum.action?rjhmId=${datePlan.rjhmId}&pjdid=${dy.pjdid}&jwdCode=${jwdCode}" target="_blank">${dy.pjNum}</a>&nbsp;&nbsp;
 		          </c:forEach>
 		        </c:if>
 		      </td>
 		      </tr>
